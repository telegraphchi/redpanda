...
class WriteCachingFailureInjectionTest(RedpandaTest):
    ...
    def test_crash_all(self):
        ...
        final_consumer = KgoVerifierSeqConsumer.oneshot(
            self.test_context, self.redpanda, self.topic_name, loop=False
        )
        assert (
            total_produced - total_lost
            == final_consumer.consumer_status.validator.valid_reads
        ), "unexpected number of consumed messages"
    ...
    def test_leader_epoch(self):
        ...
        for attempt_ix in range(MAX_ATTEMPTS):
            ...
            KgoVerifierProducer.oneshot(
                self.test_context, self.redpanda, self.topic_name, self.MSG_SIZE, 1
            )
            initial_leader_epoch = next(
                self.rpk.describe_topic(self.topic)
            ).leader_epoch
            ...
            if lost == 0:
                ...
                self.logger.info(f"No data loss observed. Retrying {attempt_ix=}.")
                continue
        ...
        assert False, (
            "Failed to observe data loss after multiple attempts. "
    ...
    def test_unavoidable_data_loss(self):
        ...
        KgoVerifierProducer.oneshot(
            self.test_context, self.redpanda, self.topic_name, self.MSG_SIZE, 5000
        )
        ...
        third_quorum_hwm = wait_until_result(
        ...
        assert third_quorum_hwm == expected_hwm, (
            "Data loss observed after network partition healing."
        )
