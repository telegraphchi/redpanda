...
class TopicDeleteCloudStorageTest(RedpandaTest):
    ...
    topics = (
        TopicSpec(
            partition_count=partition_count, cleanup_policy=TopicSpec.CLEANUP_DELETE
        ),
    )
    ...
    def _populate_topic(self, topic_name, nodes: Optional[list] = None, spillover=True):
        """
    ...
    def drop_lifecycle_marker_test(self, cloud_storage_type):
        ...
        with firewall_blocked(self.redpanda.nodes, self._s3_port):
            ...
            controller_markers = admin.get_cloud_storage_lifecycle_markers()["markers"]
            ...
            admin.delete_cloud_storage_lifecycle_marker(
                marker["topic"], marker["revision_id"], node=controller_node
            )
            ...
            def update_propagated():
                ...
                """
                nodes = [n for n in self.redpanda.nodes if n != controller_node]
                return all(
                    len(admin.get_cloud_storage_lifecycle_markers(node=n)["markers"])
                    == 0
                    for n in nodes
                )
            ...
            self.redpanda.wait_until(update_propagated, timeout_sec=10, backoff_sec=0.5)
    ...
    def topic_delete_unavailable_test(self, cloud_storage_type):
        ...
        with firewall_blocked(self.redpanda.nodes, self._s3_port):
            ...
            wait_until(
                ...
                timeout_sec=90,
                backoff_sec=10,
                err_msg="Local storage purge did not complete while cloud storage was unavailable",
            )
        ...
        wait_until(
            lambda: topic_storage_purged(self.redpanda, next_topic),
            timeout_sec=35,
            backoff_sec=1,
        )
    ...
    def _validate_topic_deletion(
        self, topic_name: str, cloud_storage_type: CloudStorageType
    ):
        if cloud_storage_type == CloudStorageType.ABS:
            ...
            assert len(sizes) > 1 and (sizes[0] > sizes[-1] or sizes[-1] == 0), (
            ...
            wait_until(
                lambda: self._topic_remote_deleted_entirely(topic_name),
                timeout_sec=60,
                backoff_sec=10,
            )
    ...
    def _assert_topic_lifecycle_marker_status(
        self, topic_name: str, status: LifecycleMarkerStatus
    ):
        """
        ...
        if status == LifecycleMarkerStatus.PURGED:
            ...
            def get_topics_with_markers():
                admin = Admin(self.redpanda)
                markers = admin.get_cloud_storage_lifecycle_markers()["markers"]
                self.logger.info(f"markers: {markers}")
                return set(i["topic"] for i in markers)

            wait_until(
                lambda: topic_name not in get_topics_with_markers(),
                timeout_sec=10,
                backoff_sec=1,
            )
    ...
    @skip_debug_mode  # Rely on timely uploads during leader transfers
    @cluster(num_nodes=5)
    @matrix(disable_delete=[False, True], cloud_storage_type=get_cloud_storage_type())
    def topic_delete_cloud_storage_test(self, disable_delete, cloud_storage_type):
        if disable_delete:
    ...
    def partition_movement_test(self, cloud_storage_type):
        ...
        size_before = sum(
            bucket_view.cloud_log_size_for_ntp(self.topic, i).total(no_archive=True)
            for i in range(0, self.partition_count)
        )
        ...
        replacement_node = next(
            iter(
                (
                    set([self.redpanda.idx(n) for n in self.redpanda.nodes])
                    - set(nodes_before)
                )
            )
        )
