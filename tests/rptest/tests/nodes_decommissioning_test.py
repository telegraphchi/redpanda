...
class NodesDecommissioningTest(PreallocNodesTest):
    ...
    def _wait_until_status(self, node_id, status, timeout_sec=15, api_node=None):
    ...
    def test_decommissioning_node_rf_1_replica(self):
        ...
        self.logger.info(
            f"decommissioning node: {node_id}",
        )
        ...
        self.redpanda.start_node(
            node=to_decommission, auto_assign_node_id=True, omit_seeds_on_idx_one=False
        )
    ...
    def test_decommissioning_cancel_ongoing_movements(self, cloud_topic: bool):
        ...
        details = self.admin.get_partitions(topic=tp_to_move, partition=p_to_move)
        ...
        self.admin.set_partition_replicas(
            topic=tp_to_move, partition=p_to_move, replicas=new_replicas
        )
        # moving partition should be present in moving list
        wait_until(lambda: self._partitions_moving(), timeout_sec=15, backoff_sec=1)
    ...
    def test_learner_gap_metrics(self):
        ...
        assert calculate_total_learners_gap() == 0, (
            "when there are no pending partition movements the reported gap should be equal to 0"
        )
        ...
        def learner_gap_reported(decommissioned_node_id: int):
            ...
            assert total_gap < total_size, (
                "gap can not be larger than the size of partitions"
            )
        ...
        wait_until(
            lambda: learner_gap_reported(to_decommission_id),
            timeout_sec=60,
            backoff_sec=1,
        )
    ...
    def test_recommissioning_node(self, cloud_topic: bool):
        ...
        wait_until(lambda: self._partitions_not_moving(), timeout_sec=15, backoff_sec=1)
    ...
    def test_recommissioning_do_not_stop_all_moves_node(self, cloud_topic: bool):
        ...
        details = self.admin.get_partitions(topic=to_move_tp, partition=to_move_p)

        new_replicas = self._find_replacement(details["replicas"], to_decommission)
        self.logger.info(
            f"moving partition {to_move_tp}/{to_move_p} - {details['replicas']} -> {new_replicas}"
        )

        self.admin.set_partition_replicas(
            topic=to_move_tp, partition=to_move_p, replicas=new_replicas
        )
        # moving partition should be present in moving list
        wait_until(lambda: self._partitions_moving(), timeout_sec=15, backoff_sec=1)
    ...
    def test_recommissioning_one_of_decommissioned_nodes(self, cloud_topic: bool):
        ...
        self.logger.info(
            f"decommissioning node: {to_decommission_2}",
        )
        self._decommission(to_decommission_2, node=survivor_node)

        self._wait_until_status(to_decommission_1, "draining", api_node=survivor_node)
        self._wait_until_status(to_decommission_2, "draining", api_node=survivor_node)
        ...
        self._wait_until_status(to_decommission_1, "active", api_node=survivor_node)
    ...
    def test_decommissioning_rebalancing_node(self, shutdown_decommissioned):
        ...
        self.redpanda.add_extra_rp_conf({"retention_local_trim_interval": 5_000})
        ...
        wait_until(
            lambda: self.producer.produce_status.acked > (self.msg_count / 2),
            timeout_sec=240,
            backoff_sec=1,
        )
        ...
        wait_until(
            lambda: self._partitions_moving(node=first_node),
            timeout_sec=30,
            backoff_sec=1,
        )
    ...
    def test_flipping_decommission_recommission(
        ...
        for i in range(1, 10):
            ...
            if self._node_removed(node_id, survivor_node):
                break
            self.logger.info(
                f"recommissioning node: {node_id}",
            )
    ...
    def test_multiple_decommissions(self, cloud_topic: bool):
        ...
        for i in range(0, iteration_count):
            for b in self.redpanda.nodes:
                ...
                def has_partitions():
                    id = self.redpanda.node_id(node=decom_node, force_refresh=True)
    ...
    def test_node_is_not_allowed_to_join_after_restart(
        ...
        def tried_to_join():
            ...
            return (
                sum([1 for l in logs.splitlines() if "Error joining cluster" in l]) >= 3
            )
    ...
    def test_decommissioning_and_upgrade(self):
        ...
        for v in self.upgrade_through_versions(
            versions_in=versions, auto_assign_node_id=True
        ):
            if v == versions[0]:
                self._create_topics()
                ...
                self.redpanda.clean_node(
                    to_decommission, preserve_logs=True, preserve_current_install=True
                )
                self.redpanda.start_node(to_decommission, auto_assign_node_id=True)
    ...
    def test_recycle_all_nodes(self, auto_assign_node_id):
        ...
        self.producer.wait_for_acks(0.2 * self.msg_count, timeout_sec=60, backoff_sec=2)

        current_replicas = {self.redpanda.node_id(n) for n in self.redpanda.nodes}
        left_to_decommission = [self.redpanda.node_id(n) for n in self.redpanda.nodes]
        ...
        def cluster_view_is_consistent():
            ...
            for n in self.redpanda.started_nodes():
                ...
                controller = admin.get_partition(
                    ns="redpanda", topic="controller", id=0, node=n
                )
                controller_replicas = {r["node_id"] for r in controller["replicas"]}
                self.logger.info(
        ...
        while len(left_to_decommission) > 0:
            ...
            if len(left_to_decommission) == (len(self.redpanda.nodes) - 1):
                ...
                self.redpanda.wait_for_controller_snapshot(
                    self.redpanda.controller(), prev_start_offset=0, timeout_sec=60
                )
            ...
            # clean the node data
            self.redpanda.clean_node(
                n, preserve_logs=True, preserve_current_install=True
            )
...
class NodeDecommissionFailureReportingTest(RedpandaTest):
    ...
    def test_allocation_failure_reporting(self):
        ...
        self.redpanda.start(
            nodes=all_but_one, auto_assign_node_id=True, omit_seeds_on_idx_one=False
        )
        ...
        partitions_json = admin.get_partitions(topic=spec.name, node=to_decommission)
        partitions = set(
            [f"{p['ns']}/{p['topic']}/{p['partition_id']}" for p in partitions_json]
        )
        ...
        def wait_for_allocation_failures(failed_partitions):
            def wait(node):
                status = admin.get_decommission_status(to_decommission_id, node=node)
                if "allocation_failures" not in status.keys():
                    return False
                alloc_failures = set(status["allocation_failures"])
                self.logger.debug(f"alloc failures: {alloc_failures}, node: {node}")
                return failed_partitions == alloc_failures

            def check_all_nodes():
                return all(wait(n) for n in self.redpanda.started_nodes())

            self.logger.debug(f"Waiting for allocation failures: {failed_partitions}")
            wait_until(
                check_all_nodes,
                timeout_sec=60,
                backoff_sec=1,
...
class NodeDecommissionSpaceManagementTest(RedpandaTest):
    ...
    def test_decommission(self, single_partition):
        ...
        data_size = 2 * target_size * len(self.redpanda.nodes) // replication_factor
        ...
        extra_rp_conf = {
            ...
            "retention_local_target_bytes_default": segments_per_partition_trimmed
            * log_segment_size,
        ...
        rpk = RpkTool(self.redpanda)
        rpk.create_topic(
            topic_name, partitions=partition_count, replicas=replication_factor
        )
        ...
        self.logger.info(f"Produced {hmb([data_size])} in {produce_duration} seconds")
...
class NodeIdReuseTest(RedpandaTest):
    ...
    def __init__(self, test_context):
        super(NodeIdReuseTest, self).__init__(test_context=test_context, num_brokers=3)
    ...
    def test_node_status(self):
        ...
        controller_id = admin.await_stable_leader(
            "controller", namespace="redpanda", timeout_s=30
        )
        ...
        self.logger.info(f"decomming node {other_node.name} (id: {other_node_id})...")
        admin.decommission_broker(other_node_id)
        waiter = NodeDecommissionWaiter(
            self.redpanda, node_id=other_node_id, logger=self.logger
        )
        waiter.wait_for_removal()
        ...
        controller_offset = admin.get_controller_status(controller)["dirty_offset"]
        self.redpanda.wait_for_controller_snapshot(
