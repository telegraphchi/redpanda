...
ss::future<>
raft_fixture_base::stop_node(model::node_id id, remove_data_dir remove) {
...
raft_node_instance& raft_fixture_base::node(model::node_id id) {
    return *_nodes.at(id);
}
...
std::optional<model::node_id> raft_fixture_base::random_follower_id() const {
    auto leader_id = get_leader();
...
ss::future<model::node_id> raft_fixture_base::wait_for_leader_change(
...
ss::future<model::node_id>
raft_fixture_base::wait_for_leader(std::chrono::milliseconds timeout) {
    return wait_for_leader(timeout + model::timeout_clock::now());
}

std::optional<model::node_id> raft_fixture_base::get_leader() const {
...
ss::future<> raft_fixture_base::create_simple_group(size_t number_of_nodes) {
...
void raft_fixture_base::notify_replicas_on_config_change() const {
    for (const auto& [_, node] : _nodes) {
        node->raft()->notify_config_update();
    }
}

ss::future<> raft_fixture_base::disable_background_flushing() const {
    co_await ss::smp::invoke_on_all([]() {
...
ss::future<> raft_fixture_base::reset_background_flushing() const {
    co_await ss::smp::invoke_on_all([]() {
...
ss::future<> raft_fixture_base::set_write_caching(bool value) const {
