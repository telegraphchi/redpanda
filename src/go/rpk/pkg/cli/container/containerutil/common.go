...
var (
	...
	consoleImageBase  = "redpandadata/console:v3.7.4"
)
...
func GetState(ctx context.Context, c Client, nodeID uint, isConsole bool) (*NodeState, error) {
...
func RemoveNetwork(ctx context.Context, c Client) error {
...
func PullImage(ctx context.Context, c Client, img string) error {
...
func CreateProfile(ctx context.Context, fs afero.Fs, c Client, y *config.RpkYaml) error {
	if p := y.Profile(ContainerProfileName); p != nil {
		return ErrContainerProfileExists
	}
	var kaAddresses, aAddresses, srAddresses []string
	existingNodes, err := GetExistingNodes(ctx, c)
	if err != nil {
